...
6) Run `get_oauth2token()` to create an Oauth 2 token based on the specifications provided in 5).

```{r}
get_oauth2token()
```

7) When asked, whether you want to use a local file to cache OAuth access credentials between R sessions, choose "1: Yes".

8) You'll be redirected to your browser to grant access to your application. Accept and close the browser tab.
